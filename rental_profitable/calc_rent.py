...
from utils import del_msg, update_msg_to_del
from telebot.states.sync.context import StateContext
from rental_profitable.calc_rent_functions import CalculatingRentProfitable
...
class RentProfitableErrors(RentProfitableTypes, ABC):
    def __init__(self):
        RentProfitableTypes.__init__(self)
        ABC.__init__(self)


    def property_appreciation_error(self, message:Message, state:StateContext):
        chat_id = message.chat.id
        user_id = message.from_user.id
        state.set(UserStates.waiting_price_of_house_for_rent)
        del_msg(chat_id, user_id)
        to_del1 = bot.send_message(chat_id, ' Упс, Хьюстон, у нас проблемы. Неверный формат ввода, пожалуйста, повторите ввод.\n Или обратитесь к специалисту.', reply_to_message_id=message.id)
        to_del2 = bot.send_message(chat_id, 'Введите оценку стоимости недвижимости $/м²')
        to_del3 = bot.send_message(message.chat.id, 'Меню:', reply_markup=self.create_keyboard1())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2, to_del3])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    def property_appreciation_error_big_price(self, message:Message, state:StateContext):
        chat_id = message.chat.id
        user_id = message.from_user.id
        state.set(UserStates.waiting_price_of_house_for_rent)
        del_msg(chat_id, user_id)
        to_del1 = bot.send_message(chat_id, 'Я думаю у вас есть что обсудить со специалистом.\n Учитывая низкую ликвидность объектов такого рода, я не смогу дать точную оценку. \n Вы можете оставить заявку или позвонить на круглосуточную линию, если вам нужен индивидуальный подход.')
        to_del2 = bot.send_message(chat_id, 'Или можете продолжить, если указали неверную сумму.\n Для этого введите оценку стоимости недвижимости $/м², ещё раз.')
        to_del3 = bot.send_message(message.chat.id, 'Меню:', reply_markup=self.create_keyboard2())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2, to_del3])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    def apartment_area_error(self, message:Message, state:StateContext):
        chat_id = message.chat.id
        user_id = message.from_user.id
        del_msg(chat_id, user_id)
        ...
        message_doh_ar_er = bot.send_message(chat_id, 'Меню:', reply_markup=self.create_keyboard3())
        cache_storage.set_value(f'r_ar_er_del:{chat_id}', message_doh_ar_er.message_id)
        cache_storage.set_value(f'reminder_states:{chat_id}', 'r_in_progress')


    def forecast_period_error(self, message:Message, state:StateContext):
        chat_id = message.chat.id
        user_id = message.from_user.id
        del_msg(chat_id, user_id)
        state.set(UserStates.waiting_forcast_period_for_rent)
        to_del1 = bot.send_message(chat_id, 'Упс, Хьюстон, у нас проблемы. Неверный формат ввода, пожалуйста, повторите ввод.', reply_to_message_id=message.id)
        to_del2 = bot.send_message(message.chat.id, 'Введите срок прогнозирования (количество лет)')
        to_del3 = bot.send_message(message.chat.id, 'Меню:', reply_markup=self.create_keyboard5())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2, to_del3])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    def rental_price_error(self, message:Message, state:StateContext):
        chat_id = message.chat.id
        user_id = message.from_user.id
        state.set(UserStates.waiting_rental_price_for_rent)
        to_del1 = bot.send_message(chat_id, ' Упс, Хьюстон, у нас проблемы. \n Неверный формат ввода, пожалуйста, повторите ввод.')
        to_del2 = bot.send_message(chat_id, 'Введите ставку арендной платы $/месяц')
        to_del3 = bot.send_message(chat_id, 'Меню:', reply_markup=self.create_keyboard6())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2, to_del3])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    def maintenance_cost_error(self, message:Message, state:StateContext):
        chat_id = message.chat.id
        user_id = message.from_user.id
        del_msg(chat_id, user_id)
        state.set(UserStates.waiting_maintance_cost_for_rent)
        to_del1 = bot.send_message(chat_id, ' Упс, Хьюстон, у нас проблемы. Возникла ошибка, пожалуйста, повторите ввод.')
        to_del2 = bot.send_message(chat_id, 'Введите затраты на содержание $ в год, учтите необходимость периодического космитического ремонта ремонта и обновление устаревшей техники')
        to_del3 = bot.send_message(chat_id, 'Меню:', reply_markup=self.create_keyboard7())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2, to_del3])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


class RentProfitable(RentProfitableTypes, RentProfitableErrors):
    instance=None
    ...
    def send_greeting(self, chat_id, user_id):
        del_msg(chat_id, user_id)
        with open('./content/rent/hello.jpg', 'rb') as img1:
            to_del1 = bot.send_photo(chat_id, img1, caption='Привет, я телеграм-бот, который рассчитывает выгодность сдачи квартиры по доходному методу. Я буду задавать вам несколько вопросов, а потом покажу вам результаты. Вы можете начать сначала, если захотите, или перейти на мой тг аккаунт или сайт, если заинтереваны')
        to_del2 = bot.send_message(chat_id, 'Меню', reply_markup=self.create_keyboard1())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_started')


    def property_appreciation(self, message:Message, state:StateContext):
        chat_id, user_id = message.chat.id, message.from_user.id
        del_msg(chat_id, user_id)
        state.set(UserStates.waiting_price_of_house_for_rent)
        with open('./content/rent/price.jpg', 'rb') as img:
            to_del1 = bot.send_photo(chat_id, img, caption='Введите оценку стоимости недвижимости. Если вы её не знаете, то можете воспользоваться \n функцией рассчёта рыночной стоимости.')
        to_del2 = bot.send_message(message.chat.id, 'Меню:', reply_markup=self.create_keyboard1())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2])
        cache_storage.set_value(f'reminder_states:{chat_id}', 'r_in_progress')


    @bot.message_handler(state=UserStates.waiting_price_of_house_for_rent)
    def prop_ap_hand(self, message:Message):
        chat_id, user_id = message.chat.id, message.from_user.id
        del_msg(chat_id, user_id)
        if not message.text:
        ...
        try:
            ...
            return self.property_appreciation_error(message)
        cache_storage.set_value(user_id, chat_id, 'r_property_appreciation', r_answer)
        self.apartment_area(chat_id, user_id)


    def apartment_area(self, chat_id, user_id, state:StateContext):
        state.set(UserStates.waiting_apartment_area_for_rent)
        del_msg(chat_id, user_id)
        with open('./content/rent/area.jpg', 'rb') as img1:
            to_del1 = bot.send_photo(chat_id, img1, caption='Введите площадь квартиры м²')
        to_del2 = bot.send_message(chat_id, 'Меню:', reply_markup=self.create_keyboard3())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    @bot.message_handler(state = UserStates.waiting_apartment_area_for_rent)
    def ap_ar_hand(self, message:Message):
        chat_id, user_id = message.chat.id, message.from_user.id
        del_msg(chat_id, user_id)
        if not message.text:
    ...
    def number_of_rooms(self, chat_id, user_id):
        del_msg(chat_id, user_id)
        with open('./content/rent/rooms.jpg', 'rb') as img1:
            to_del1 = bot.send_photo(chat_id, img1, caption='Выберите количество комнат в квартире.')
        to_del2 = bot.send_message(chat_id, 'Меню', reply_markup=self.create_keyboard4())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    def forecast_period(self, chat_id:int, user_id:int, state:StateContext):
        del_msg(chat_id, user_id)
        state.set(UserStates.waiting_forcast_period_for_rent)
        with open('./content/rent/forecast.jpg', 'rb') as img1:
            to_del1 = bot.send_photo(chat_id, img1, 'Введите срок прогнозирования (количество лет)')
        to_del2 = bot.send_message(chat_id, 'Меню:', reply_markup=self.create_keyboard5())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    @bot.message_handler(state=UserStates.waiting_forcast_period_for_rent)
    def r_for_per_hand(self, message:Message):
        chat_id, user_id = message.chat.id, message.from_user.id
        if not message.text:
    ...
    def rental_price(self, chat_id:int, user_id:int, state:StateContext):
        del_msg(chat_id, user_id)
        state.set(UserStates.waiting_rental_price_for_rent)
        with open('./content/rent/income.jpg', 'rb') as img1:
            to_del1 = bot.send_photo(chat_id, img1, caption='Введите ставку арендной платы $/месяц')
        to_del2 = bot.send_message(chat_id, 'Меню:', reply_markup=self.create_keyboard6())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    @bot.message_handler(state = UserStates.waiting_rental_price_for_rent)
    def r_rent_price_hand(self, message:Message):
        chat_id, user_id = message.chat.id, message.from_user.id
        del_msg(chat_id, user_id)
        if not message.text:
            return self.property_appreciation_error(message)
    ...
    def maintenance_cost(self, chat_id:int, user_id:int, state:StateContext):
        del_msg(chat_id, user_id)
        state.set(UserStates.waiting_maintance_cost_for_rent)
        with open('./content/rent/costs.jpg', 'rb') as img1:
            to_del1 = bot.send_photo(chat_id, img1, caption='Введите затраты на содержание $ в год, учтите необходимость периодического космитического ремонта ремонта и обновление устаревшей техники')
        to_del2 = bot.send_message(chat_id, 'Меню:', reply_markup=self.create_keyboard7())
        update_msg_to_del(chat_id, user_id, [to_del1, to_del2])
        cache_storage.set_value(user_id, chat_id, 'reminder_states', 'r_in_progress')


    @bot.message_handler(state = UserStates.waiting_maintance_cost_for_rent)
    def r_the_end(self, message:Message):
        chat_id, user_id = message.chat.id, message.from_user.id
        del_msg(chat_id, user_id)
        if not message.text:
    ...
    def send_results(self, chat_id:int, user_id:int):
        ...
        if results['r_is_profitable'] == True:
            ...
            with open('./content/rent/result.jpg', 'rb') as img1:
                to_del1 = bot.send_photo(chat_id, img1, caption=message1 + message2 + message3, parse_mode='Markdown') 
        else:
            message = f"**Оценка, с учётом того, что за планируемый срок сдачи цена не изменится.** \n\n Настоящая стоимость квартиры по рыночному методу: {int(results['r_property_appreciation'])} $.\n"
            message1 += f"Это стоимость, которую вы можете получить от продажи квартиры сейчас, учитывая ожидаемую рыночную стоимость и оценку стоимости недвижимости."
            message2 = f"Планируемый доход через {int(results['r_forecast_period'])} лет с учетом продажи квартиры: {int(results['r_present_apartment_value'], 500)} $.\n"
            message2 += f"Это доход, который вы можете получить от сдачи квартиры в аренду за {int(results['r_forecast_period'])} лет и продажи ее по истечении этого срока, учитывая дисконтирование, инфляцию, ставку вакансии и затраты на содержание."
            ...
            with open('./content/rent/result.jpg', 'rb') as img1:
                to_del1 = bot.send_photo(chat_id, img1, caption=message1 + message2 + message3, parse_mode='Markdown') 
        if results['r_is_profitable_up'] == True:
            ...
            message3 = f"\n Сдача квартиры в аренду выгодна.\n"
            message3 += f"Это значит, что планируемый доход от сдачи и продажи квартиры больше, чем настоящая стоимость квартиры. Вы можете получить дополнительный доход от аренды квартиры, пока она растет в цене."
            to_del2 = bot.send_message(chat_id, message1 + message2 + message3, parse_mode='Markdown')
        else:
            message1 = f"**Оценка, с учётом того, что за планируемый срок сдачи цены на недвижимость будут расти.** \n\n"
            message2 = f"Планируемый доход через {int(results['r_forecast_period'])} лет с учетом продажи квартиры: {int(results['r_present_apartment_value_up'], 500)} $.\n"
            message2 += f"Это доход, который вы можете получить от сдачи квартиры в аренду за {int(results['r_forecast_period'])} лет и продажи ее по истечении этого срока, учитывая дисконтирование, инфляцию, ставку вакансии и затраты на содержание."
            ...
            message3 += f"Это значит, что планируемый доход от сдачи и продажи квартиры меньше, чем настоящая стоимость квартиры. Вы можете потерять часть своего капитала, если будете сдавать квартиру в аренду, вместо того, чтобы продать ее сейчас."
            to_del2 = bot.send_message(chat_id, message1 + message2 + message3, parse_mode='Markdown')
        if results['r_is_profitable_down'] == True:
            ...
            message3 += f"Это значит, что планируемый доход от сдачи и продажи квартиры меньше, чем настоящая стоимость квартиры. Вы можете потерять часть своего капитала, если будете сдавать квартиру в аренду, вместо того, чтобы продать ее сейчас."
            bot.send_message(chat_id, message1 + message2 + message3, parse_mode='Markdown')
        msg = bot.send_message(chat_id, 'Меню', reply_markup=self.create_keyboard8())
        cache_storage.set_value(f'r_rsr_del:{chat_id}', msg.message_id) 
